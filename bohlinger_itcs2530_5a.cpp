...
void ShowUsage()
{
    ...
    //output header and instructions
...
void MakePurchase(int& iTotalSmall, int& iTotalMedium, int& iTotalLarge, int& iTotalXXXS)
{
    ...
    int i;
    string s;
    //output prompt and recieve input
    ...
    cin >> i >> s;
    //decision structure to decide which variable to operate on
    if (s == "S" || s == "s")
    {
        iTotalSmall += i;
    }
...
{
    //multiple if statements to decide what information to output
    if (iTotalSmall > 0)
    {
...
void DisplayTotal(const int iTotalSmall, const int iTotalMedium, const int iTotalLarge, const int iTotalXXXS)
{
    ...
    const double ds = 175, dm = 190, dl = 200, dx = 50;
